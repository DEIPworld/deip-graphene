...
public:
    ...
    get_contents_refer_to_content(const research_content_id_type& research_content_id) const
    {
        ...
        const auto& idx = db->get_index<research_content_reference_operations_history_index>().indices().get<by_research_content_reference>().equal_range(research_content_id);
...
std::vector<applied_research_content_reference_operation>
research_content_reference_history_api::get_contents_refer_to_content(
    const research_content_id_type& research_content_id) const
{
    const auto db = _impl->_app.chain_database();
    return db->with_read_lock([&]() {
        return _impl->get_contents_refer_to_content<research_content_reference_operations_history_object>(
            research_content_id);
    });
}
