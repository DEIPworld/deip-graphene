...
public:
    ...
    virtual void maybe_get_private_key(fc::optional<fc::ecc::private_key>& result,
        const deip::chain::public_key_type& pubkey, const std::string& account_name)
        = 0;
};

class debug_node_plugin : public deip::app::plugin
{
public:
    ...
    void debug_update(Lambda&& callback, uint32_t skip = deip::chain::database::skip_nothing)
    {
    ...
    uint32_t debug_generate_blocks(const std::string& debug_key, uint32_t count,
        uint32_t skip = deip::chain::database::skip_nothing, uint32_t miss_blocks = 0,
    ...
    uint32_t debug_generate_blocks_until(const std::string& debug_key, const fc::time_point_sec& head_block_time,
        bool generate_sparsely, uint32_t skip = deip::chain::database::skip_nothing,
        private_key_storage* key_storage = nullptr);
