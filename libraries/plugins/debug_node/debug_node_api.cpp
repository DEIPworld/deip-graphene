...
public:
    ...
    virtual void maybe_get_private_key(fc::optional<fc::ecc::private_key>& result,
                                       const deip::chain::public_key_type& pubkey,
                                       const std::string& account_name) override;

    std::string dev_key_prefix;
    std::map<deip::chain::public_key_type, fc::ecc::private_key> key_table;
};

class debug_node_api_impl
{
public:
    debug_node_api_impl(deip::app::application& _app);
    ...
    fc::optional<deip::chain::signed_block> debug_pop_block();
    // void debug_push_block( const deip::chain::signed_block& block );
    deip::chain::witness_schedule_object debug_get_witness_schedule();
    deip::chain::hardfork_property_object debug_get_hardfork_property_object();
    ...
    std::shared_ptr<deip::plugin::debug_node::debug_node_plugin> get_plugin();
                                                      ...
                                                      const std::string& account_name)
{
    ...
    if ((pubkey == deip::chain::public_key_type()) || (gen_pub == pubkey))
    {
        result = gen_priv;
        return;
    }
...
debug_node_api_impl::debug_node_api_impl(deip::app::application& _app)
    : app(_app)
{
#ifdef DEIP_INIT_PRIVATE_KEY
    fc::ecc::private_key init_key = DEIP_INIT_PRIVATE_KEY;
...
debug_node_api_impl::debug_push_blocks(const std::string& src_filename, uint32_t count, bool skip_validate_invariants)
{
    ...
    std::shared_ptr<deip::chain::database> db = app.chain_database();
    ...
    if (fc::exists(src_path) && fc::exists(index_path) && !fc::is_directory(src_path) && !fc::is_directory(index_path))
    {
        ...
        uint32_t skip_flags = deip::chain::database::skip_nothing;
        if (skip_validate_invariants)
            skip_flags = skip_flags | deip::chain::database::skip_validate_invariants;
        for (uint32_t i = 0; i < count; i++)
        {
            // fc::optional< deip::chain::signed_block > block = log.read_block( log.get_block_pos( first_block + i )
            // );
            uint64_t block_pos = log.get_block_pos(first_block + i);
            if (block_pos == deip::chain::block_log::npos)
            {
...
{
    return get_plugin()->debug_generate_blocks(debug_key, count, deip::chain::database::skip_nothing, 0,
                                               &key_storage);
}
                                                          ...
                                                          bool generate_sparsely)
{
    return get_plugin()->debug_generate_blocks_until(debug_key, head_block_time, generate_sparsely,
                                                     deip::chain::database::skip_nothing, &key_storage);
}

fc::optional<deip::chain::signed_block> debug_node_api_impl::debug_pop_block()
{
    std::shared_ptr<deip::chain::database> db = app.chain_database();
    return db->fetch_block_by_number(db->head_block_num());
}

/*void debug_node_api_impl::debug_push_block( const deip::chain::signed_block& block )
{
   app.chain_database()->push_block( block );
}*/

deip::chain::witness_schedule_object debug_node_api_impl::debug_get_witness_schedule()
{
    return app.chain_database()->get(deip::chain::witness_schedule_id_type());
}

deip::chain::hardfork_property_object debug_node_api_impl::debug_get_hardfork_property_object()
{
    return app.chain_database()->get(deip::chain::hardfork_property_id_type());
}
...
std::shared_ptr<deip::plugin::debug_node::debug_node_plugin> debug_node_api_impl::get_plugin()
{
...
void debug_node_api_impl::debug_set_hardfork(uint32_t hardfork_id)
{
    ...
    if (hardfork_id > DEIP_NUM_HARDFORKS)
        return;
...
{
    return app.chain_database()->get(deip::chain::hardfork_property_id_type()).last_hardfork >= hardfork_id;
}

} // detail

debug_node_api::debug_node_api(const deip::app::api_context& ctx)
{
...
fc::optional<deip::chain::signed_block> debug_node_api::debug_pop_block()
{
    return my->debug_pop_block();
}

/*void debug_node_api::debug_push_block( deip::chain::signed_block& block )
{
   my->debug_push_block( block );
}*/

deip::chain::witness_schedule_object debug_node_api::debug_get_witness_schedule()
{
    return my->debug_get_witness_schedule();
}

deip::chain::hardfork_property_object debug_node_api::debug_get_hardfork_property_object()
{
