    ...
    void operator()(const custom_json_operation& op) const
    {
        try
        {
            if (op.id == FOLLOW_PLUGIN_NAME)
            {
                ...
                std::shared_ptr<custom_operation_interpreter> eval
                    = _plugin.database().get_custom_json_evaluator(op.id);
                eval->apply(new_cop);
            }
        }
        FC_CAPTURE_AND_RETHROW()
...
//            {
//                // Rule #2: If you are down voting another user, you must have more reputation than them to impact their
//                // reputation
...
//                // Rule #2: If you are down voting another user, you must have more reputation than them to impact their
//                // reputation
//                if (cv->rshares < 0 && !(voter_rep != rep_idx.end() && voter_rep->reputation > author_rep->reputation))
//                    return;
//
...
follow_plugin::follow_plugin(application* app)
    : plugin(app)
    , my(new detail::follow_plugin_impl(*this))
{
}
