...
{
public:
    raw_block_api_impl(deip::app::application& _app);

    std::shared_ptr<deip::plugin::raw_block::raw_block_plugin> get_plugin();

    deip::app::application& app;
};

raw_block_api_impl::raw_block_api_impl(deip::app::application& _app)
    : app(_app)
{
}

std::shared_ptr<deip::plugin::raw_block::raw_block_plugin> raw_block_api_impl::get_plugin()
{
...
raw_block_api::raw_block_api(const deip::app::api_context& ctx)
{
...
get_raw_block_result raw_block_api::get_raw_block(get_raw_block_args args)
{
    ...
    std::shared_ptr<deip::chain::database> db = my->app.chain_database();
    ...
    result.raw_block
        = fc::base64_encode(std::string(&serialized_block[0], &serialized_block[0] + serialized_block.size()));
