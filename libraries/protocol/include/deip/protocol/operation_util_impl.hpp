...
struct get_operation_name
{
    ...
    typedef void result_type;
    template <typename T> void operator()(const T& v) const { name = name_from_type(fc::get_typename<T>::name()); }
};
}
...
{
    typedef void result_type;
    template <typename T> void operator()(const T& v) const { v.validate(); }
};
...
struct operation_get_required_auth_visitor
{
    ...
    operation_get_required_auth_visitor(flat_set<account_name_type>& a, flat_set<account_name_type>& own,
        flat_set<account_name_type>& post, std::vector<authority>& oth)
        : active(a)
        , owner(own)
        , posting(post)
        , other(oth)
...
#define DEFINE_OPERATION_TYPE(OperationType)                                                                           \
    namespace fc {                                                                                                     \
                                                                                                                       \
    void to_variant(const OperationType& var, fc::variant& vo) { var.visit(from_operation(vo)); }                      \
                                                                                                                       \
    ...
    namespace protocol {                                                                                               \
                                                                                                                       \
    void operation_validate(const OperationType& op) { op.visit(deip::protocol::operation_validate_visitor()); }     \
                                                                                                                       \
    void operation_get_required_authorities(const OperationType& op, flat_set<account_name_type>& active,              \
        flat_set<account_name_type>& owner, flat_set<account_name_type>& posting, std::vector<authority>& other)       \
    {                                                                                                                  \
