...
{
    validate_account_name(author);
    FC_ASSERT(percent_scrs <= deip_100_PERCENT, "Percent cannot exceed 100%");
    FC_ASSERT(max_accepted_payout.symbol == deip_SYMBOL, "Max accepted payout must be in SCR");
...
void escrow_transfer_operation::validate() const
{
    ...
    FC_ASSERT(fee.symbol == deip_SYMBOL, "fee must be SCR");
    FC_ASSERT(deip_amount.symbol == deip_SYMBOL, "deip amount must contain SCR");
...
void create_budget_operation::validate() const
{
    ...
    FC_ASSERT(is_asset_type(balance, deip_SYMBOL), "Balance must be SCR");
