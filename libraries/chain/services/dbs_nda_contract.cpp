                                                    ...
                                                    const fc::time_point_sec& end_date)
{
    const auto& contract = db_impl().create<nda_contract_object>([&](nda_contract_object& c_o) {
        ...
        c_o.status = static_cast<uint16_t>(nda_contract_status::nda_contract_pending);
    });
...
const bool dbs_nda_contract::nda_contract_exists(const nda_contract_id_type& id) const
{
...
const dbs_nda_contract::contract_optional_ref_type dbs_nda_contract::get_contract_if_exists(const nda_contract_id_type& id) const
{
    contract_optional_ref_type result;
...
{
    db_impl().modify(contract, [&](nda_contract_object& c_o) { c_o.status = static_cast<uint16_t>(status); });
}

void dbs_nda_contract::process_nda_contracts()
{
    const auto& idx = db_impl().get_index<nda_contract_index>().indices().get<by_end_date>();
    auto itr = idx.begin();
    ...
    while (itr != idx.end())
    {
        if (itr->end_date <= _head_block_time) {
            auto& contract = *itr;
            set_new_contract_status(contract, nda_contract_status::nda_contract_expired);
        }
        itr++;
    }
}
