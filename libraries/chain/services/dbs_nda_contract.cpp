...
dbs_nda_contract::contracts_refs_type dbs_nda_contract::get_by_creator_research_group_and_signee_research_group(const research_group_id_type& party_a_research_group_id, const research_group_id_type& signee_research_group_id)
{
    contracts_refs_type ret;
    ...
    auto it_pair = db_impl()
                       ...
                       .equal_range(boost::make_tuple(party_a_research_group_id, signee_research_group_id));
    auto it = it_pair.first;
    const auto it_end = it_pair.second;
    while (it != it_end)
    {
    ...
    const fc::string& hash)
{
    ...
    auto it_pair = db_impl()
                       ...
                       .equal_range(boost::make_tuple(party_a_research_group_id, signee_research_group_id, hash));
    auto it = it_pair.first;
    const auto it_end = it_pair.second;
    while (it != it_end)
    {
...
const nda_contract_object& dbs_nda_contract::sign(const nda_contract_object& contract, const account_name_type& contract_signer, const fc::string& sig)
{
    db_impl().modify(contract, [&](nda_contract_object& c_o) {
        ...
        {
            fc::from_string(is_party_a_sig ? c_o.party_a_signature : c_o.signee_signature, sig);
        }
        else
        {
            ...
            fc::from_string(is_party_a_sig ? c_o.party_a_signature : c_o.signee_signature,
