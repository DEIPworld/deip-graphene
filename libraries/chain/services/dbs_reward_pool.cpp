...
void dbs_reward_pool::increase_reward_pool(const reward_pool_object& research_reward_pool, const share_type delta)
{
    FC_ASSERT(delta >= 0, "Cannot update research reward pool (delta <= 0)");
    db_impl().modify(research_reward_pool, [&](reward_pool_object& rrp_o) { rrp_o.reward_share += delta; });
}
...
void dbs_reward_pool::increase_expertise_pool(const reward_pool_object& research_reward_pool, const share_type delta)
{
    ...
    db_impl().modify(research_reward_pool, [&](reward_pool_object& rrp_o) { rrp_o.expertise_share += delta; });
}
