...
#define DEIP_DAEMON_DEFAULT_CONFIG_FILE_NAME "config.ini"
...
{
    deip::plugin::initialize_plugin_factories();
    ...
    try
    {
        ...
        //        std::cerr << "------------------------------------------------------\n";
        //        auto initdelegate_private_key = graphene::utilities::key_to_wif(DEIP_INIT_PRIVATE_KEY);
        //        std::cerr << "initdelegate public key: " << DEIP_INIT_PUBLIC_KEY_STR << "\n";
        //        std::cerr << "initdelegate private key: " << initdelegate_private_key << "\n";
        //        std::cerr << "chain id: " << std::string(DEIP_CHAIN_ID) << "\n";
        //        std::cerr << "blockchain version: " << fc::string(DEIP_BLOCKCHAIN_VERSION) << "\n";
        //        std::cerr << "------------------------------------------------------\n";
        //#else
        std::cerr << "------------------------------------------------------\n\n";
        std::cerr << "            STARTING DEIP NETWORK\n\n";
        std::cerr << "------------------------------------------------------\n";
        //        std::cerr << "initdelegate public key: " << DEIP_INIT_PUBLIC_KEY_STR << "\n";
        //        std::cerr << "chain id: " << std::string(DEIP_CHAIN_ID) << "\n";
        std::cerr << "blockchain version: " << fc::string(DEIP_BLOCKCHAIN_VERSION) << "\n";
        ...
        // clang-format off
        bpo::options_description app_options("Deip Daemon");
        bpo::options_description cfg_options("Deip Daemon");
        ...
        for (const std::string& plugin_name : deip::plugin::get_available_plugins())
            node->register_abstract_plugin(deip::plugin::create_plugin(plugin_name, node));
        ...
        fc::path config_ini_path = data_dir / DEIP_DAEMON_DEFAULT_CONFIG_FILE_NAME;
