...
target_link_libraries( {plugin_provider}_{plugin_name} deip_app deip_chain deip_protocol )
...
{{
   public:
      {plugin_name}_api( const deip::app::api_context& ctx );
...
class {plugin_name}_plugin : public deip::app::plugin
{{
   public:
      {plugin_name}_plugin( deip::app::application* app );
...
{{
   public:
      {plugin_name}_api_impl( deip::app::application& _app );
...
{plugin_name}_api_impl::{plugin_name}_api_impl( deip::app::application& _app ) : app( _app )
{{}}
...
{plugin_name}_api::{plugin_name}_api( const deip::app::api_context& ctx )
{{
...
{{
   public:
      {plugin_name}_plugin_impl( deip::app::application& app );
...
{plugin_name}_plugin_impl::{plugin_name}_plugin_impl( deip::app::application& app )
  : _app(app) {{}}
...
{plugin_name}_plugin::{plugin_name}_plugin( deip::app::application* app )
   : plugin(app)
{{
...
DEIP_DEFINE_PLUGIN( {plugin_name}, {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin )
""",
}
...
def main(argv):
    ...
    parser.add_argument("provider", help="Name of plugin provider (deip for plugins developed by Deip)")
