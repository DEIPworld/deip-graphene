...
DEIPD_PID=`pgrep -f p2p-endpoint`
if [[ ! $? -eq 0 ]]; then
  echo NOTIFYALERT! deipdsync has quit unexpectedly, checking for coredump and then starting a new instance..
  sleep 30
  SAVED_PID=`cat /tmp/deipdpid`
  if [[ -e /tmp/core.$SAVED_PID ]]; then
    gdb --batch --quiet -ex "thread apply all bt full" -ex "quit" /usr/local/deipd-full/bin/deipd /tmp/core.$SAVED_PID >> /tmp/stacktrace
    STACKTRACE=`cat /tmp/stacktrace`
    echo NOTIFYALERT! deipdsync stacktrace from coredump:
  ...
  RUN_SV_PID=`pgrep -f /etc/service/deipd`
  kill -9 $RUN_SV_PID
fi
...
if [[ ! -z "$BLOCKCHAIN_TIME" ]]; then
  ...
  if [[ ${BLOCK_AGE} -le 10 ]]; then
    DEIPD_PID=`pgrep -f p2p-endpoint`
    kill -SIGINT $DEIPD_PID
    echo deipdsync: waiting for deipd to exit cleanly
    while [ -e /proc/$DEIPD_PID ]; do sleep 0.1; done
    echo deipdsync: starting a new blockchainstate upload operation
    cd ${COMPRESSPATH:-$HOME}
    echo deipdsync: compressing blockchainstate...
    tar cf blockchain.tar.bz2 --use-compress-prog=pbzip2 -C $HOME blockchain
    if [[ ! $? -eq 0 ]]; then
      echo NOTIFYALERT! deipdsync was unable to compress shared memory file, check the logs.
      exit 1
    fi
    FILE_NAME=blockchain-$VERSION-`date '+%Y%m%d-%H%M%S'`.tar.bz2
    echo deipdsync: uploading $FILE_NAME to $S3_BUCKET
    aws s3 cp blockchain.tar.bz2 s3://$S3_BUCKET/$FILE_NAME
    if [[ ! $? -eq 0 ]]; then
    	echo NOTIFYALERT! deipdsync was unable to upload $FILE_NAME to s3://$S3_BUCKET
    	exit 1
    fi
    echo deipdsync: replacing current version of blockchain-latest.tar.bz2 with $FILE_NAME
    ...
    if [[ ! $? -eq 0 ]]; then
    	echo NOTIFYALERT! deipdsync was unable to overwrite the current blockchainstate with $FILE_NAME
    	exit 1
    fi
    ...
    echo deipdsync: stopping the container after a sync operation
    if [[ -e /tmp/isnewsync ]]; then
      echo notifydeipdsync: deipdsync: successfully generated and uploaded new blockchain-$VERSION-latest.tar.bz2 to s3://$S3_BUCKET
    fi
    RUN_SV_PID=`pgrep -f /etc/service/deipd`
    kill -9 $RUN_SV_PID
  fi
