...
BOOST_AUTO_TEST_CASE( vests_stock_split )
{
   try
   {
      ...
      auto old_vesting_fund = db.get_dynamic_global_properties().common_tokens_fund;
...
com_itr->net_rshares.value * magnitude * magnitude : 0;
      ...
      BOOST_REQUIRE( db.get_dynamic_global_properties().common_tokens_fund == old_vesting_fund );
