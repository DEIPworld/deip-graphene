...
public:
    ...
    void create_proposals()
    {
        db.create<proposal_object>([&](proposal_object& d) {
            d.id = 1;
            d.action = research_group_quorum_action::dropout_member;
            d.data = "1";
            d.creator = "alice";
        ...
        db.create<proposal_object>([&](proposal_object& d) {
            d.id = 2;
            d.action = research_group_quorum_action::invite_member;
            d.data = "123";
            d.creator = "bob";
        ...
        db.create<proposal_object>([&](proposal_object& d) {
            d.id = 3;
            d.action = research_group_quorum_action::change_quorum;
            d.data = "1123john";
            d.creator = "john";
        ...
        db.create<proposal_object>([&](proposal_object& d) {
            d.id = 4;
            d.action = research_group_quorum_action::change_research_review_share_percent;
            d.data = "1123";
            d.creator = "alice";
            d.research_group_id = 1;
            d.creation_time = db.head_block_time();
...
BOOST_AUTO_TEST_CASE(create_proposal)
{
    try
    {
        const proposal_object& proposal = data_service.create_proposal(research_group_quorum_action::invite_member,
                "{\"name\":\"bob\",\"research_group_id\":31,\"research_group_token_amount_in_percent\":5000}", "alice",
                                                                       1, fc::time_point_sec(0xffffffff), 40);

        BOOST_CHECK(proposal.action == research_group_quorum_action::invite_member);
        ...
        BOOST_CHECK(proposal.quorum == 40);
...
BOOST_AUTO_TEST_CASE(get_proposal_by_id)
 {
     try
     {
         ...
         BOOST_CHECK(proposal.action == research_group_quorum_action::change_quorum);
         ...
         BOOST_CHECK(proposal.quorum == 72);
    ...
    try
    {
        ...
        BOOST_CHECK(std::any_of(proposals.begin(), proposals.end(), [](std::reference_wrapper<const proposal_object> wrapper){
            ...
            return  proposal.id == 1 && proposal.research_group_id == 1 &&
                    proposal.action == research_group_quorum_action::dropout_member &&
                    proposal.data == "1" &&
        ...
        BOOST_CHECK(std::any_of(proposals.begin(), proposals.end(), [](std::reference_wrapper<const proposal_object> wrapper){
            ...
            return  proposal.id == 4 && proposal.research_group_id == 1 &&
                    proposal.action == research_group_quorum_action::change_research_review_share_percent &&
                    proposal.data == "1123" &&
                    proposal.creator == "alice" &&
...
BOOST_AUTO_TEST_CASE(vote_for_proposal)
 {
     try
     {
         ...
         research_group_service.add_member_to_research_group("alice", 1, 10, account_name_type());
         auto& token = research_group_service.get_research_group_token_by_account_and_research_group("alice", 1);
