...
BOOST_AUTO_TEST_CASE(approve_research_group_invite_apply)
{
    try
    {
        ...
        research_group_create_by_operation("alice", "name rg1", "permlink rg1", "description rg1", DEIP_100_PERCENT, proposal_quorums, true, false);
        research_group_create_by_operation("alice", "name rg2", "permlink rg2", "description rg2", DEIP_100_PERCENT, proposal_quorums, true, false);
...
BOOST_AUTO_TEST_CASE(reject_research_group_invite_apply)
{
    try
    {
        ...
        research_group_create(31, "name", "permlink", "description", 200, proposal_quorums, true, false);
...
BOOST_AUTO_TEST_CASE(research_update_apply)
{
    try {
        ...
        research_group_create(31, "name", "permlink", "description", 100, proposal_quorums, true, false);
...
BOOST_AUTO_TEST_CASE(send_funds_execute_test)
{
    try {
        ...
        setup_research_group(31, "name", "research_group", "research group", 750, proposal_quorums, true, false, accounts);
...
{
    BOOST_TEST_MESSAGE("Testing: create_research_material");
...
BOOST_AUTO_TEST_CASE(check_dgpo_used_power)
{
    try
    {
        ...
        research_group_create(30, "group3", "test3", "test3", 100, proposal_quorums, true, false);
...
BOOST_AUTO_TEST_CASE(add_member_to_research_test)
{
    try
    {
        BOOST_TEST_MESSAGE("Testing: add_member_to_research");
        ...
        db.create<research_object>([&](research_object& r) {
            ...
            r.title = "title";
            r.permlink = "permlink";
            r.research_group_id = alice_rg.id;
            r.review_share_in_percent = 1000;
            r.dropout_compensation_in_percent = 500;
            r.is_finished = false;
            r.created_at = db.head_block_time();
        ...
        add_member_to_research_operation op;
        op.research_id = 30;
        op.owner = "alice";
        op.invitee = "mike";
        ...
        auto& rgt = db.get<research_group_token_object, by_owner>(std::make_tuple("alice", alice_rg.id));

        BOOST_CHECK(rgt.amount == 9900);

        auto& rgt2 = db.get<research_group_token_object, by_owner>(std::make_tuple("mike", alice_rg.id));

        BOOST_CHECK(rgt2.amount == 100);

        auto& research = db.get<research_object>(30);

        BOOST_CHECK(research.members.size() == 2);

        add_member_to_research_operation op2;
        op2.research_id = 30;
        op2.owner = "mike";
        op2.invitee = "jack";
        ...
        tx2.set_expiration(db.head_block_time() + DEIP_MAX_TIME_UNTIL_EXPIRATION);
        tx2.operations.push_back(op2);
        tx2.sign(mike_priv_key, db.get_chain_id());
        tx2.validate();
        BOOST_CHECK_THROW(db.push_transaction(tx2, 0), fc::assert_exception);
    }
    FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE(exclude_member_from_research_test)
{
    try
    {
        BOOST_TEST_MESSAGE("Testing: exclude_member_from_research");
        ...
        db.create<research_group_token_object>([&](research_group_token_object& rgt_o) {
            rgt_o.id = 101;
            rgt_o.research_group_id = alice_rg.id;
            rgt_o.amount = 1;
            rgt_o.owner = "bob";
        });
        ...
        exclude_member_from_research_operation op;
        op.research_id = 30;
        op.owner = "bob";
        op.account_to_exclude = "alice";
        ...
        exclude_member_from_research_operation op2;
        op2.research_id = 30;
        op2.owner = "alice";
        op2.account_to_exclude = "bob";
        ...
        BOOST_CHECK(research.members.size() == 1);
    }
    FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE(create_contract_test)
{
    try
    {
        ...
        BOOST_CHECK(subscription.current_nda_contract_quota_units == 0);
        BOOST_CHECK(subscription.extra_nda_contract_quota_units == 2);
    }
    FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE(request_contract_file_key_test)
{
    try
    {
        ...
        auto& rgt = db.create<research_group_token_object>([&](research_group_token_object& rgt_o) {
            rgt_o.id = 50;
            rgt_o.research_group_id = 0;
            rgt_o.amount = 1;
            rgt_o.owner = "bob";
        });
        ...
        op.contract_id = 0;
        op.requester = "bob";
        op.encrypted_payload_hash = "f6fd3b373dfa10280cee23224d0a34cdb02869ae15ceb6bd306861fc482bcc2e";
        op.encrypted_payload_iv = "6eb5eaaed5d54b2f75e475e5b5f6b52e";
        ...
        BOOST_CHECK(request.encrypted_payload_hash == "f6fd3b373dfa10280cee23224d0a34cdb02869ae15ceb6bd306861fc482bcc2e");
        BOOST_CHECK(request.encrypted_payload_iv == "6eb5eaaed5d54b2f75e475e5b5f6b52e");
        BOOST_CHECK(request.encrypted_payload_encryption_key == "");
    }
    FC_LOG_AND_RETHROW()
}

BOOST_AUTO_TEST_CASE(fulfill_request_by_nda_contract)
{
    try
    {
        BOOST_TEST_MESSAGE("Testing: fulfill_request_by_nda_contract");
        ...
        BOOST_CHECK(subscription.current_nda_protected_file_quota_units == 9);
    }
    FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE(update_subscription)
{
    try
    {
        BOOST_TEST_MESSAGE("Testing: update_subscription");
