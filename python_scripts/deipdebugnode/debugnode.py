...
from deipapi.deipnoderpc import DeipNodeRPC

class DebugNode( object ):
   """ Wraps the deipd debug node plugin for easier automated testing of the Deip Network"""

   def __init__( self, deipd, data_dir, args='', plugins=[], apis=[], deipd_out=None, deipd_err=None ):
      """ Creates a deipd debug node.
      ...
      For all other requests, the python-deip library should be used.

      args:
         deipd -- The string path to the location of the deipd binary
         data_dir -- The string path to an existing deipd data directory which will be used to pull blocks from.
         args -- Other string args to pass to deipd.
         ...
         deipd_stdout -- A stream for deipd's stdout. Default is to pipe to /dev/null
         deipd_stderr -- A stream for deipd's stderr. Default is to pipe to /dev/null
      """
      self._data_dir = None
      self._debug_key = None
      self._FNULL = None
      self._rpc = None
      self._deipd_bin = None
      self._deipd_lock = None
      self._deipd_process = None
      self._temp_data_dir = None

      self._deipd_bin = Path( deipd )
      if( not self._deipd_bin.exists() ):
         raise ValueError( 'deipd does not exist' )
      if( not self._deipd_bin.is_file() ):
         raise ValueError( 'deipd is not a file' )

      self._data_dir = Path( data_dir )
      if( not self._data_dir.exists() ):
         raise ValueError( 'data_dir either does not exist or is not a properly constructed deip data directory' )
      ...
      if( deipd_out != None ):
         self.deipd_out = deipd_out
      else:
         self.deipd_out = self._FNULL

      if( deipd_err != None ):
         self.deipd_err = deipd_err
      else:
         self.deipd_err = self._FNULL

      self._debug_key = '5JHNbFNDg834SFj8CMArV6YW7td4zrPzXveqTfaShmYVuYNeK69'
      self._deipd_lock = Lock()


   def __enter__( self ):
      self._deipd_lock.acquire()
      ...
      deipd = [ str( self._deipd_bin ), '--data-dir=' + str( self._temp_data_dir.name ) ]
      deipd.extend( self._args )

      self._deipd_process = Popen( deipd, stdout=self.deipd_out, stderr=self.deipd_err )
      self._deipd_process.poll()
      sleep( 5 )
      if( not self._deipd_process.returncode ):
         self._rpc = DeipNodeRPC( 'ws://127.0.0.1:8095', '', '' )
      else:
         raise Exception( "deipd did not start properly..." )

   def __exit__( self, exc, value, tb ):
      self._rpc = None

      if( self._deipd_process != None ):
         self._deipd_process.poll()

         if( not self._deipd_process.returncode ):
            self._deipd_process.send_signal( SIGINT )

            sleep( 7 )
            self._deipd_process.poll()

            if( not self._deipd_process.returncode ):
               self._deipd_process.send_signal( SIGTERM )

               sleep( 5 )
               self._deipd_process.poll()

               if( self._deipd_process.returncode ):
                  loggin.error( 'deipd did not properly shut down after SIGINT and SIGTERM. User intervention may be required.' )

      self._deipd_process = None
      self._temp_data_dir.cleanup()
      self._temp_data_dir = None
      self._deipd_lock.release()
   ...
   def debug_generate_blocks( self, count ):
      ...
      `get_dev_key deip debug`. Do not use this key on the live chain for any reason.
   ...
   def debug_set_hardfork( self, hardfork_id ):
      ...
      args:
         hardfork_id: The id of the hardfork to set. Hardfork IDs start at 1 (0 is genesis) and increment
            by one for each hardfork. The maximum value is deip_NUM_HARDFORKS in chain/hardfork.d/0-preamble.hf
      """
      if( hardfork_id < 0 ):
...
if __name__=="__main__":
   ...
   def main():
      global WAITING
      """
      This example contains a simple parser to obtain the locations of both deipd and the data directory,
      ...
      parser = ArgumentParser( description='Run a Debug Node on an existing chain. This simply replays all blocks ' + \
                                 ...
                                 'the CLI wallet' )
      parser.add_argument( '--deipd', '-s', type=str, required=True, help='The location of a deipd binary to run the debug node' )
      ...
      deipd = Path( args.deipd )
      if( not deipd.exists() ):
         print( 'Error: deipd does not exist.' )
         return

      deipd = deipd.resolve()
      if( not deipd.is_file() ):
         print( 'Error: deipd is not a file.' )
         return

      data_dir = Path( args.data_dir )
      if( not data_dir.exists() ):
         print( 'Error: data_dir does not exist or is not a properly constructed deipd data directory' )
      ...
      debug_node = DebugNode( str( deipd ), str( data_dir ), deipd_err=sys.stderr )
